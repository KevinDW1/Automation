...
KNOWN_CUSTOMER = {"id": "16338", "name": "Precision Impact Development"}
...
class TestResult:
    ...
    def ok(self, msg):   self.logs.append(f"  ✓ {msg}"); print(f"  ✓ {msg}")
    def fail(self, msg): self.failure_reasons.append(msg); self.logs.append(f"  ✗ {msg}"); print(f"  ✗ {msg}")
    def log(self, msg):  self.logs.append(f"    {msg}"); print(f"    {msg}")
    def print_report(self):
        print(f"\n{'='*65}\n  {self.test_id} -- {self.title}\n{'-'*65}")
        print(f"  VERDICT: {'PASS' if self.passed else 'FAIL'}")
        [print(f"    • {r}") for r in self.failure_reasons]
        print("="*65)
...
async def dismiss_discard_dialog(page: Page) -> None:
    ...
    for sel in ["button:has-text('Yes')", "button:has-text('Continue')",
                "button:has-text('Discard')", "button:has-text('OK')"]:
...
async def close_any_modal(page: Page) -> None:
    await page.wait_for_timeout(400)
    await page.keyboard.press("Escape")
    await page.wait_for_timeout(400)
    await dismiss_discard_dialog(page)
    await page.evaluate("() => { const c = document.querySelector('.oc-modal-curtain'); if(c) c.remove(); }")
...
async def click_new_customer(page: Page) -> None:
    clicked = await page.evaluate("""() => {
        for (const b of document.querySelectorAll('button')) {
            if (/new customer/i.test(b.textContent.trim())) { b.click(); return true; }
        }
        return false;
    }""")
    if not clicked:
...
async def fill_placeholder(page: Page, placeholder: str, value: str, log_name: str) -> bool:
    for ph in [placeholder, placeholder.rstrip(" *")]:
...
async def fill_by_label(page: Page, label: str, value: str, log_name: str) -> None:
    await fill_placeholder(page, label, value, log_name)
...
async def fill_dropdown(page: Page, placeholder: str, value: str, log_name: str) -> None:
    ph_bare = placeholder.rstrip(" *")
    clicked = await page.evaluate(
        """(args) => {
            for (const ph of args) {
                const inp = document.querySelector('input[placeholder=' + JSON.stringify(ph) + ']');
                if (inp) {
                    const w = inp.closest('.e-ddl') || inp.parentElement;
                    if (w) { w.dispatchEvent(new MouseEvent('click', {bubbles:true})); return true; }
                }
            }
            return false;
        }""",
    ...
    await page.wait_for_timeout(700)
    opt = page.locator(f"[role='listbox'] li:has-text('{value}')").first
    if await opt.count() > 0 and await opt.is_visible():
        await opt.click()
        print(f"  ✓ {log_name} selected: {value}")
        await page.wait_for_timeout(300)
        return
    opt2 = page.locator("[role='listbox'] li.e-list-item:first-child").first
    if await opt2.count() > 0 and await opt2.is_visible():
        await opt2.click()
        print(f"  ✓ {log_name} selected first item")
        return
    await page.keyboard.press("Enter")
    print(f"  ✓ {log_name} entered via Enter")
...
async def fill_step1(page: Page, data: dict) -> None:
    ...
    await page.evaluate("""() => {
        for (const b of document.querySelectorAll('button')) {
            if (/^next$/i.test(b.textContent.trim())) { b.click(); return; }
        }
    }""")
    await page.wait_for_timeout(2_000)
    await wait_spinners_gone(page)
...
async def fill_step2(page: Page, data: dict) -> None:
    ...
    await page.evaluate("""() => {
        for (const b of document.querySelectorAll('button')) {
            if (/^create$/i.test(b.textContent.trim())) { b.click(); return; }
        }
    }""")
...
async def dismiss_open_popups(page: Page) -> None:
    for sel in ["button:has-text('Close')", "button[aria-label='Close']", ".e-dlg-closeicon-btn"]:
        btn = page.locator(sel).first
        if await btn.count() > 0 and await btn.is_visible():
            await btn.click()
            await page.wait_for_timeout(300)

async def get_all_text_blocks(page: Page) -> list:
...
async def get_grid_rows_text(page: Page) -> list:
    return await page.evaluate("""() =>
